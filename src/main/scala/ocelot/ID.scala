...
/**
 * Generic type to represent all kinds of identifiers in Ocelot. The type parameter refers to
 ...
 * [[EventID#auto]], as illustrated below:
 * {{{
 *   case class Token(
 *       from: PID,
 *       to: PID,
 *       id: EventID = EventID.auto()
 *     ) extends UnicastMessage(from, to, id)
 * }}}
 *
 * @param value     real unique identifier value (as UUID)
 */
case class EventID(value: UUID = UUID.randomUUID()) extends ID[UUID] with Ordered[EventID]
{
  type SameType = EventID

  def compare (that: EventID): Int = this.value.compareTo(that.value)

  def name = s"m($value)"

  override def toString = this.name

  protected def idWith(newID: UUID): EventID = if (value == newID) this else copy(value = newID)
}

/**
 * identifier of a task obtained from scheduling an action through an instance of [[kernel.Scheduler]].
...
object TaskID
{
  ...
  protected[ocelot] def auto() : TaskID = {
    lastID += 1
    TaskID(lastID)
  }
}
...
object SignalID
{
  def auto(): SignalID = SignalID(UUID.randomUUID())
}
